...
- (void)viewDidLayoutSubviews {
    ...
    // The code below disables scroll view y contnet offset, otherwise the page
    // will have a top offset.
    // If page scroll need to be enabled, need to do this in
    // XLPagerTabStripViewController viewDidScroll method
    self.scrollView.contentOffset =
    CGPointMake(self.scrollView.contentOffset.x, 0);
}
...
- (void)subscribNotifications {
    ...
    [[NSNotificationCenter defaultCenter]
     addObserver:self
     selector:@selector(showCreateTeamView)
     name:kEventNoTeam
     object:nil];
}
...
(XLPagerTabStripViewController *)pagerTabStripViewController {
    ...
    CLAHomeTopicViewController *homeTopicViewController = [storyBoard
                                                           instantiateViewControllerWithIdentifier:kHomeTopicViewController];
    
    CLAHomeMemberViewController *homeMemberViewController = [storyBoard
                                                             instantiateViewControllerWithIdentifier:kHomeMemberViewController];
    
    CLAHomeNotificationViiewController *notificationViewController =
    [[CLAHomeNotificationViiewController alloc] init];
    
    return @[
             homeTopicViewController,
             homeMemberViewController,
