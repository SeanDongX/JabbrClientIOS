...
{
    XLFormDescriptor *form = [XLFormDescriptor formDescriptorWithTitle:NSLocalizedString(@"Topic Settings", nil)];
    ...
    row = [XLFormRowDescriptor formRowDescriptorWithTag:kTopicName rowType:XLFormRowDescriptorTypeName title:[self.roomViewModel.room getHandle]];
    row.disabled = @YES;
    [section addFormRow:row];
    
    row = [XLFormRowDescriptor formRowDescriptorWithTag:kLeaveTopicButton rowType:XLFormRowDescriptorTypeButton title:NSLocalizedString(@"Leave", nil)];
    ...
    if (notMembers.count > 0) {
        ...
        row = [XLFormRowDescriptor formRowDescriptorWithTag:kSendInviteButton rowType:XLFormRowDescriptorTypeButton title:NSLocalizedString(@"Send Invite", nil)];
        [row.cellConfig setObject:[Constants highlightColor] forKey:kTextLabelColor];
    ...
    for (NSString *key in  keys) {
        if ([key hasPrefix:kInvitePrefix]) {
            ...
            if (value != [NSNull null] && [value boolValue]) {
                NSString *username = [key stringByReplacingOccurrencesOfString:kInvitePrefix withString:@""];
                
                if (username && username.length > 0) {
                    matchFound = true;
                    [[CLASignalRMessageClient sharedInstance] inviteUser:username inRoom:self.roomViewModel.room.name];
                }
            }
        }
    }
