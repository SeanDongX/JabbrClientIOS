...
- (void)viewWillAppear:(BOOL)animated {
  ...
  // TODO: Bug - progress bar not shown, mostly like conflict with
  // ECSlidingViewontroller
...
- (void)initProgressDelegate {
  ...
  _progressView.progressBarView =
      [[NJKWebViewProgressView alloc] initWithFrame:barFrame];
  _progressView.autoresizingMask =
...
- (void)loadDocumentThread:(DocumentThread *)documentThread {
  ...
  if (documentThread) {
    ...
    if (self.currentDocumentThread.url) {
      NSURLRequest *req =
          [[NSURLRequest alloc] initWithURL:self.currentDocumentThread.url];
