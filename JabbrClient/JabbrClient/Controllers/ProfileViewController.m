...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[self.signOutButton layer]
     setBorderColor:[Constants mainThemeColor].CGColor];
    
    [self.signOutButton setTitleColor:[Constants mainThemeColor]
    ...
    [userIcon addAttribute:NSForegroundColorAttributeName
                     value:[UIColor whiteColor]];
    
    NSString *initial = [self getUserRealName].length > 1
    ?[[self getUserRealName] substringToIndex:1]
    : @"";
    
    [self.profileImageView
     setImage:
     [JSQMessagesAvatarImageFactory
      avatarImageWithUserInitials:[initial uppercaseString]
...
- (NSString *)getUserRealName {
    ...
    if (username != nil) {
        CLAUser *user = [
                         ...
                         findUser:username];
        if (user != nil && user.realName != (id)[NSNull null] &&
            user.realName.length != 0) {
