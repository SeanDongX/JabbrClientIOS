                            ...
                            text:(NSString *)text
{
    ...
    self = [self initWithOId:oId SenderId:senderId senderDisplayName:senderDisplayName date:date isMedia:NO];
    if (self) {
        _text = [text copy];
    }
    return self;
                           ...
                           media:(id<JSQMessageMediaData>)media
{
    ...
    self = [self initWithOId:oId SenderId:senderId senderDisplayName:senderDisplayName date:date isMedia:YES];
    if (self) {
        _media = media;
    }
    return self;
...
{
    return [NSString stringWithFormat:@"<%@: oId=%@, senderId=%@, senderDisplayName=%@, date=%@, isMediaMessage=%@, text=%@, media=%@>",
            [self class], self.oId, self.senderId, self.senderDisplayName, self.date, @(self.isMediaMessage), self.text, self.media];
}
