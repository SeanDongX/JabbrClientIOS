...
- (id) init
{
    ...
    self.hub = [[SBNotificationHub alloc] initWithConnectionString: kAzureNotificationHubConnectionString                                                              notificationHubPath: kAuzreNotificationHubName];
    return self;
}
...
- (void)registerDevice {
    ...
    [self.hub registerNativeWithDeviceToken:deviceToken tags:tagSet completion:^(NSError* error) {
        if (error != nil) {
            NSLog(@"Error registering device token for notifications: %@", error);
        }
    }];
}
...
- (void)unregisterDevice {
    ...
    [self.hub unregisterAllWithDeviceToken:deviceToken completion:^(NSError* error) {
        if (error != nil) {
